...
function addon:Options()
    ...
    local amtEditBox = CreateFrame("EditBox", nil, panel, "InputBoxTemplate")
    amtEditBox:SetPoint("TOPLEFT", sotf, "BOTTOMLEFT", 0, -16)  -- Place it below the last control for layout
    amtEditBox:SetSize(100, 30)  -- Width, Height
    amtEditBox:SetAutoFocus(false)
    amtEditBox:SetText(tostring(addon.db.staticAmtNeeded)) -- Set initial value from addon.db
    amtEditBox:SetScript("OnTextChanged", function(self)
       local val = tonumber(self:GetText())
       if val then
           addon.db.staticAmtNeeded = val
           -- Update Code A logic here, if needed
       end
    end)
    
    local amtLabel = panel:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
    amtLabel:SetPoint("LEFT", amtEditBox, "RIGHT", 10, 0)
    amtLabel:SetText("Static Amount Needed:")
